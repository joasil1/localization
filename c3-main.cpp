...
#define CHANNELS_V FOV_V/RESOLUTION // #
#define CHANNELS_H FOV_H/RESOLUTION // #
#define POINTS_SCAN CHANNELS_H*CHANNELS_V // # Scan once per rotation
#define POINTS_RATE POINTS_SCAN*ROTATION_RATE // Hz - we could direcly take but since we 
//
...
int main(){
	...
	lidar_bp.SetAttribute("upper_fov", to_string(FOV_UPPER));
    lidar_bp.SetAttribute("lower_fov", to_string(FOV_LOWER));
    lidar_bp.SetAttribute("channels", to_string(CHANNELS_V));
    lidar_bp.SetAttribute("range", to_string(RANGE)); 
	lidar_bp.SetAttribute("rotation_frequency", to_string(ROTATION_RATE));
	lidar_bp.SetAttribute("points_per_second", to_string(POINTS_RATE));
	...
	while (!viewer->wasStopped())
  	{
		...
		if(!new_scan){
			...
			voxGrid.setInputCloud (scanCloud);
			voxGrid.setLeafSize(0.05, 0.05, 0.05); // 5cm edge
			...
			PointCloudT::Ptr cloudMatched (new PointCloudT);
			pcl::transformPointCloud (*cloudFiltered, *cloudMatched, transform);

			viewer->removePointCloud("scan");
			viewer->removePointCloud("scan_filtered");
			viewer->removePointCloud("scan_matched");

			// TODO: Change `scanCloud` below to your transformed scan
			renderPointCloud(viewer, scanCloud, "scan", Color(1,0,0) );
			renderPointCloud(viewer, cloudFiltered, "scan_filtered", Color(0,1,0) );
			renderPointCloud(viewer, cloudMatched, "scan_matched", Color(0,1,0) );
